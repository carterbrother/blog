...
public interface StockMngMapper {
    @Select(
        "select T1.productCode, T1.ownedCount, T0.count  as stockCount," +
                " T0.name,T0.price,T0.description " +
                " from  stock_tb T0  join (select stock_tb.product_code as productCode, sum(user_orders.count) as ownedCount " +
            ...
            "group by stock_tb.product_code) AS T1" +
                " on T0.product_code = T1.productCode ")
