package com.spring.histroy.demo.reactor.exception.compare;
 ...
 */
@Component
public class CustomWebsocketRoutingFilter1 implements GlobalFilter, Ordered {

    public static final String SEC_WEBSOCKET_PROTOCOL = "Sec-WebSocket-Protocol";
    private static final Log log = LogFactory.getLog(CustomWebsocketRoutingFilter1.class);
    private final WebSocketClient webSocketClient;
    private final WebSocketService webSocketService;
    private final ObjectProvider<List<HttpHeadersFilter>> headersFiltersProvider;
    private volatile List<HttpHeadersFilter> headersFilters;

    public CustomWebsocketRoutingFilter1(WebSocketClient webSocketClient, WebSocketService webSocketService, ObjectProvider<List<HttpHeadersFilter>> headersFiltersProvider) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (!ServerWebExchangeUtils.isAlreadyRouted(exchange) && ("ws".equals(scheme) || "wss".equals(scheme))) {
            ...
            return this.webSocketService.handleRequest(exchange, new CustomWebsocketRoutingFilter1.ProxyWebSocketHandler(requestUrl, this.webSocketClient, filtered, protocols));
        } else {
    ...
    private static class ProxyWebSocketHandler implements WebSocketHandler {
        ...
        public Mono<Void> handle(WebSocketSession session) {
            return this.client.execute(this.url, this.headers, new WebSocketHandler() {
                ...
                @Override
                public List<String> getSubProtocols() {
                    return CustomWebsocketRoutingFilter1.ProxyWebSocketHandler.this.subProtocols;
                }
            });
        }
    }
